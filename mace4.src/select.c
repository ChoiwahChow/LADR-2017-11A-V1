...
void selection_measure(int id, int *max, int *max_id) {
	...
	case MOST_OCCURRENCES:
		n = num_occurrences(id);
		break;
	case MOST_PROPAGATIONS:
		n = num_propagations(id);
		break;
	case MOST_CONTRADICTIONS:
		n = num_contradictions(id, *max);
		break;
	case MOST_CROSSED:
		n = num_crossed(id);
		break;
	default:
		fatal_error("selection_measure: bad selection measure");
	}
	if (n > *max) {
		*max = n;
		*max_id = id;
...
int select_cell(int max_constrained) {
	...
	case SELECT_LINEAR:
		id = select_linear(0, First_skolem_cell - 1);
		break;
	case SELECT_CONCENTRIC:
		id = select_concentric(0, First_skolem_cell - 1);
		break;
	case SELECT_CONCENTRIC_BAND:
		id = select_concentric_band(0, First_skolem_cell - 1, max_constrained);
		break;
	default:
		fatal_error("bad selection order");
	}
	...
	switch (parm(Opt->selection_order)) {
	case SELECT_LINEAR:
		id = select_linear(First_skolem_cell, Number_of_cells - 1);
		break;
	case SELECT_CONCENTRIC:
		id = select_concentric(First_skolem_cell, Number_of_cells - 1);
		break;
	case SELECT_CONCENTRIC_BAND:
		id = select_concentric_band(First_skolem_cell, Number_of_cells - 1,
				max_constrained);
		break;
	default:
		fatal_error("bad selection order");
