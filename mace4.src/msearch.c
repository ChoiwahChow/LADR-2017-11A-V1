...
void init_mace_options(Mace_options opt) {
	opt->domain_size = init_parm("domain_size", 0, 0, INT_MAX);   // init_parm(name, default_value, min, max)
	...
	opt->print_models_gap = init_flag("print_models_gap", FALSE);
	...
	opt->iterate = init_stringparm("iterate", 5, "all", "evens", "odds",
			"primes", "nonprimes");

	/* dependencies */

	flag_flag_dependency(opt->print_models_gap, TRUE, opt->print_models_tabular,
			FALSE);
	flag_flag_dependency(opt->print_models_gap, TRUE, opt->print_models, FALSE);
	flag_flag_dependency(opt->print_models_tabular, TRUE, opt->print_models,
			FALSE);
	flag_flag_dependency(opt->print_models, TRUE, opt->print_models_tabular,
			FALSE);

	flag_flag_dependency(opt->iterate_primes, TRUE, opt->iterate_nonprimes,
			FALSE);
	flag_flag_dependency(opt->iterate_nonprimes, TRUE, opt->iterate_primes,
			FALSE);
	...
	flag_stringparm_dependency(opt->iterate_primes, TRUE, opt->iterate,
			"primes");
	flag_stringparm_dependency(opt->iterate_nonprimes, TRUE, opt->iterate,
			"nonprimes");
...
char* exit_string(int code) {
	char *message;
	switch (code) {
	case MAX_MODELS_EXIT:
		message = "max_models";
		break;
	case ALL_MODELS_EXIT:
		message = "all_models";
		break;
	case EXHAUSTED_EXIT:
		message = "exhausted";
		break;
	case MAX_MEGS_YES_EXIT:
		message = "max_megs_yes";
		break;
	case MAX_MEGS_NO_EXIT:
		message = "max_megs_no";
		break;
	case MAX_SEC_YES_EXIT:
		message = "max_sec_yes";
		break;
	case MAX_SEC_NO_EXIT:
		message = "max_sec_no";
		break;
	case MACE_SIGINT_EXIT:
		message = "mace_sigint";
		break;
	case MACE_SIGSEGV_EXIT:
		message = "mace_sigsegv";
		break;
	default:
		message = "???";
	}
...
void mace4_exit(int exit_code) {
	...
	fprintf(stderr, "\n------ process %d exit (%s) ------\n", my_process_id(),
			exit_string(exit_code));
	printf("\nProcess %d exit (%s) %s", my_process_id(), exit_string(exit_code),
			get_date());
	...
	if (flag(Opt->integer_ring)) {
		...
		for (s = Symbols; s != NULL; s = s->next) {
			...
			if (is_symbol(s->sn, "+", 2)) {
				for (i = 0; i < Domain_size; i++)
					for (j = 0; j < Domain_size; j++)
						Cells[X2(s->base, i, j)].value = Domain[(i + j)
								% Domain_size];
				...
				for (i = 0; i < Domain_size; i++)
					Cells[X1(s->base, i)].value = Domain[(Domain_size - i)
							% Domain_size];
			} else if (is_symbol(s->sn, "--", 2)) {
				for (i = 0; i < Domain_size; i++)
					for (j = 0; j < Domain_size; j++)
						Cells[X2(s->base, i, j)].value = Domain[((i
...
static BOOL check_that_ground_clauses_are_true(void) {
	Plist g;
	BOOL ok = TRUE;
...
int possible_model(void) {
...

	{
		...
		if (flag(Opt->print_models_gap))
			print_model_gap(stdout);
		...
		fflush(stdout);
		if (parm(Opt->max_models) != -1
				&& Total_models >= parm(Opt->max_models))
...
static BOOL mace4_skolem_check(int id) {
...
static BOOL iterate_ok(int n, char *class) {
...
Mace_results mace4(Plist clauses, Mace_options opt) {
	...
	if (rc == SEARCH_MAX_MODELS)
		...
		results->return_code =
				Total_models == 0 ? EXHAUSTED_EXIT : ALL_MODELS_EXIT;
	else if (rc == SEARCH_MAX_TOTAL_SECONDS)
		results->return_code =
				Total_models == 0 ? MAX_SEC_NO_EXIT : MAX_SEC_YES_EXIT;
	else if (rc == SEARCH_MAX_MEGS)
		results->return_code =
				Total_models == 0 ? MAX_MEGS_NO_EXIT : MAX_MEGS_YES_EXIT;
	else
