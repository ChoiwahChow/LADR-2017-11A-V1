...
static Ordertype compare_cells(struct cell *a, struct cell *b) {
	if (a->symbol->attribute == EQUALITY_SYMBOL
			&& b->symbol->attribute != EQUALITY_SYMBOL)
		return GREATER_THAN;
	...
	else if (Skolems_last && a->symbol->attribute == SKOLEM_SYMBOL
			&& b->symbol->attribute != SKOLEM_SYMBOL)
		return GREATER_THAN;
	...
	else if (a->max_index < b->max_index)
		return LESS_THAN;

	else if (a->max_index > b->max_index)
		return GREATER_THAN;

	else if (a->symbol->mace_sn < b->symbol->mace_sn)
		return LESS_THAN;

	else if (a->symbol->mace_sn > b->symbol->mace_sn)
		return GREATER_THAN;

	else if (sum_indexes(a->eterm) < sum_indexes(b->eterm))
		return LESS_THAN;
