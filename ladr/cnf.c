...
static Formula share_formula(Formula f, Hashtab h) {
	if (f->type == AND_FORM || f->type == OR_FORM || f->type == ALL_FORM
			|| f->type == EXISTS_FORM) {
		int i;
		for (i = 0; i < f->arity; i++)
		...
		g = hash_lookup(f, hashval, h, (BOOL (*)(void*, void*)) formula_ident);
		if (g) {
			zap_formula(f);
			g->excess_refs++;
			return g;
...
static Formula consolidate_formula(Formula f) {
...
static BOOL complementary_share(Formula a, Formula b) {
	return (a->type == NOT_FORM && formula_ident_share(a->kids[0], b))
...
static BOOL contains_complements_share(Formula f) {
	int i, j;
...
static BOOL prop_member_share(Formula f, Formula g) {
	int i;
	for (i = 0; i < g->arity; i++)
...
static BOOL prop_subset_share(Formula f, Formula g) {
	int i;
	for (i = 0; i < f->arity; i++)
...
static BOOL prop_subsume_share(Formula f, Formula g) {
	if (FALSE_FORMULA(f))
		return TRUE;
...
static Formula remove_subsumed_share(Formula f) {
	if (f->type != AND_FORM)
		return f;
	else {
		Formula h;
...
static Formula bbt(Formula f, int start, int end) {
	if (start == end)
		return f->kids[start];
	else {
		int mid = (start + end) / 2;
...
static Formula balanced_binary(Formula f) {
	if (f->type != AND_FORM && f->type != OR_FORM)
		return f;
	else if (f->arity == 0)
		return f;
...
static Formula disjoin_flatten_simplify(Formula a, Formula b) {
	Formula c;
	int new_arity, i, j;
...
static Formula simplify_and_share(Formula f) {
	if (f->type != AND_FORM)
		return f;
	else {
...
static Formula distribute_top(Formula h) {
	Formula f = h->kids[0];
	Formula g = h->kids[1];
	int arity, i, j, k;
	Formula a;
...
static Formula distribute(Formula f) {
...
static Formula skolem(Formula f, Ilist uvars) {
...
static Ilist unique_qvars(Formula f, Ilist vars) {
	if (f->type == ATOM_FORM)
		return vars;
...
static Formula remove_uni_quant(Formula f, int *varnum_ptr) {
	if (f->type == AND_FORM || f->type == OR_FORM) {
		int i;
		for (i = 0; i < f->arity; i++)
...
static Formula ms_free_vars(Formula f) {
